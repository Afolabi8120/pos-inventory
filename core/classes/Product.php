<?php
	...
	class Product extends Admin {
		...
		public function fetchAllProduct(){
			$stmt = $this->pdo->prepare("SELECT * FROM tblproduct WHERE status = 1 ORDER BY product_name ASC ");
			$stmt->execute();

			return $stmt->fetchAll(PDO::FETCH_OBJ);
		}

		public function fetchAllProductUsingCatID($cat_id){
			$stmt = $this->pdo->prepare("SELECT * FROM tblproduct WHERE status = 1 AND category_id = :value ORDER BY product_name ASC ");
			$stmt->bindParam(":value", $cat_id, PDO::PARAM_INT);
			$stmt->execute();
        ...
        public function fetchExpiredProducts(){
        	...
        	$stmt = $this->pdo->prepare("SELECT * FROM tblproduct WHERE DATE(expiry_date) < :currentDate ");

<?php
	...
	class Order extends Admin{
		...
		public function addPayment($invoiceno,$total,$paytype,$payment_status,$amount_paid,$change){

			$stmt = $this->pdo->prepare("INSERT INTO tblpayment (invoiceno,total,paytype,payment_status,amount_paid,_change) VALUES(:invoiceno,:total,:paytype,:payment_status,:amount_paid,:change)");
			$stmt->bindParam(":invoiceno", $invoiceno, PDO::PARAM_STR);
		...
		public function getProfitSummary($date_from,$date_to){
			$stmt = $this->pdo->prepare("SELECT c.invoiceno,pro.product_name,c.user_id,c.quantity,c.price,p.total,p.paytype,p.payment_status,p.date_paid,((pro.selling_price - pro.buying_price) * c.quantity) AS profit FROM tblcart AS c INNER JOIN tblpayment AS p ON c.invoiceno = p.invoiceno INNER JOIN tblproduct AS pro ON c.product_id = pro.product_id WHERE DATE(p.date_paid) BETWEEN :date_from AND :date_to");
			$stmt->bindParam(":date_from", $date_from, PDO::PARAM_STR);
			$stmt->bindParam(":date_to", $date_to, PDO::PARAM_STR);
			$stmt->execute();
		...
		public function getProfitSummaryTotal($date_from,$date_to){
			$stmt = $this->pdo->prepare("SELECT SUM(((pro.selling_price - pro.buying_price) * c.quantity)) AS profit FROM tblcart AS c INNER JOIN tblpayment AS p ON c.invoiceno = p.invoiceno INNER JOIN tblproduct AS pro ON c.product_id = pro.product_id WHERE DATE(p.date_paid) BETWEEN :date_from AND :date_to ");
			$stmt->bindParam(":date_from", $date_from, PDO::PARAM_STR);
			$stmt->bindParam(":date_to", $date_to, PDO::PARAM_STR);
			$stmt->execute();
		...
		public function getCashierSales($user_id){
			$stmt = $this->pdo->prepare("SELECT DISTINCT(p.invoiceno),p.total,p.paytype,p.payment_status,p.amount_paid,p._change,p.date_paid FROM tblpayment AS p INNER JOIN tblcart AS c ON p.invoiceno = c.invoiceno WHERE c.user_id = :user_id ORDER BY p.date_paid DESC");

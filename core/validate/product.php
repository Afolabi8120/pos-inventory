<?php
    ...
    if(isset($_POST['btnAddProduct']) && !empty($_POST['btnAddProduct'])){
        ...
        // Form Validation 
        if(empty($product_code) || empty($product_name) || empty($barcode) || empty($unit) || empty($supplier_id) || empty($category_id) || empty($buying_price) || empty($selling_price) || empty($quantity) || empty($reorder_level) || $status == ""){
	        ...
	        $validextensions = ['jpg', 'png', 'jpeg', 'webp'];
